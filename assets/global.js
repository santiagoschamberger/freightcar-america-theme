...
debugger;
    ...
    const maxQuantity = parseInt(this.input.dataset.max) || Infinity;
    let newValue = previousValue;
    const price = this.input.dataset.current_price;

    if (event.target.name === 'plus') {
        if (isPallet && unitsPerPallet > 1) {
            newValue += unitsPerPallet;
        } else {
            newValue += parseInt(this.input.step) || 1;
        }
    } else {
        if (isPallet && unitsPerPallet > 1) {
            newValue -= unitsPerPallet;
    ...
    // Ensure the new value does not exceed the valid max quantity considering pallet increments
   // const maxAllowedPalletQty = maxQuantity - (maxQuantity % unitsPerPallet);
  // if (newValue + cartQuantity > maxAllowedPalletQty) {
  //      newValue = maxAllowedPalletQty - cartQuantity;
  //  }

    // Ensure newValue is at least the minimum value
      if (isPallet && unitsPerPallet > 1){
        
      }else{
        newValue = Math.max(newValue, parseInt(this.input.min) || 1);
      }
    

    this.input.value = newValue;
  if(price){
    const new_price =  ( newValue * parseInt(price) / 100).toFixed(2);

    if(new_price > 0){
        document.querySelector(".price-item.price-item--regular").innerHTML = '$'+new_price;      
    }else{
          document.querySelector(".price-item.price-item--regular").innerHTML = '$'+parseInt(price) / 100;
    }
...
document.addEventListener('click', function (event) {
  ...
  const minBoxes = parseInt(displayInput.getAttribute('min'), 10) || 1;

  let boxes = parseInt(displayInput.value, 10) || minBoxes;
  ...
  if (minusButton) {
    boxes = Math.max(minBoxes, boxes - 1);
  }
